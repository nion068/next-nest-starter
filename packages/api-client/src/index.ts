...
export class ApiClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    const response = await this.fetcher(`${this.options.baseUrl}/api/v1${path}`, {
      ...init,
      credentials: 'include',
  ...
  login(input: { email: string; password: string }) {
    return this.request<{ ok: true }>('/auth/login', {
      method: 'POST',
      body: JSON.stringify(input),
    });
  }
  ...
  refresh() {
    return this.request<{ ok: true }>('/auth/refresh', { method: 'POST' });
  }

  requestPhoneCode(phone: string) {
    return this.request<void>('/auth/phone/code', {
      method: 'POST',
      body: JSON.stringify({ phone }),
    });
  }

  loginPhone(input: { phone: string; code: string }) {
    return this.request<{ ok: true }>('/auth/phone/login', {
      method: 'POST',
      body: JSON.stringify(input),
    });
  }
